...
```mermaid
    classDiagram
        ...
        class Appetizer {
            - size: str
            + __init__(name, price, quantity, size)
        }
    
        class Drink {
            - type: str
            + __init__(name, price, quantity, type)
        }
    
        class MainCourse {
            - vegetarian: bool
            - family_size: bool
            + __init__(name, price, quantity, vegetarian, family_size)
        }
        ...
        Order "1" o-- "*" MenuItem
```
Y el códgo de python (también encontrado como archivo en el repositorio):
```python
